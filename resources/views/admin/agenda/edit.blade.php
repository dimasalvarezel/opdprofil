            ...
            <div class="col-md-12">
              ...
              <div class="card card-success">
                <div class="card-header">
                  <h3 class="card-title"><strong>Sunting Data Agenda</strong></h3>
                </div>
                <!-- /.card-header -->
                <!-- form start -->
                <form role="form" action="{{route('admin.agenda.update')}}" id="quickForm" method="post" enctype="multipart/form-data">
                  @csrf
                  <div class="card-body">
                    <input type="hidden" name="id" class="form-control" id="id" value="{{$agenda->id}}">
                    <div class="form-group">
                      <label for="title">Judul Agenda</label>
                      <input type="text" name="title" class="form-control" id="title" value="{{$agenda->title}}">
                    </div>
                    <div class="form-group">
                      <label for="title">Lokasi</label>
                      <input type="text" name="location" class="form-control" id="location" value="{{$agenda->location}}">
                    </div>                    
                    <div class="form-group">
                    ...
                    <div class="form-group">
                      ...
                      <div class="custom-file">
                        <input type="file" class="custom-file-input" name="img" id="img" value="{{$agenda->img}}">
                        <label class="custom-file-label" for="img">{{($agenda->img)?$agenda->img:'Unggah File'}}</label>
                      </div>
                    </div>
                    <div class="form-group">
                      ...
                      <div class="select2-green">
                        <select class="form-control select2bs4" name="status" style="width: 100%;">
                          <option value="show" @if($agenda->status == "show") selected @else "" @endif >Ya</option>
                          <option value="hide" @if($agenda->status == "hide") selected @else "" @endif >Tidak</option>
                        </select>
                      </div>
                    </div>
                    <div class="form-group">
                      <label for="description">Deskripsi</label>
                      <textarea name="description" id="description" class="textarea" required>{{$agenda->description}}</textarea>
                    </div>
                  </div>
                  <!-- /.card-body -->
