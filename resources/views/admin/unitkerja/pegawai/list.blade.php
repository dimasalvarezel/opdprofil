          ...
          <div class="col-12">
            <div class="card">
              <div class="card-header">
                <h3 class="card-title">Data Staff/Pegawai</h3>
              ...
              <div class="card-body">
                <table id="example1" class="table table-bordered table-striped">
                  ...
                  <tbody>
                    ...
                    <tr>
                      <td>{{ $i }}</td>
                      <td>{{ ($row->field_id) ? $row->field->name : '-' }}</td>
                      <td>{{ $row->name }}</td>
                      <td>{{ ($row->nip)?$row->nip:'-' }}</td>
                      <td>{{ $row->position }}</td>
                      <td>{{ $row->status }}</td>
                      <td>
                        <a href="{{route('admin.staff.show',['id' => $row->id])}}" class="btn btn-md btn-secondary btn-icon" title="Detail">
                          <i class="fas fa-info-circle"></i>
                        </a>
                        <a href="{{route('admin.staff.edit',['id' => $row->id])}}" class="btn btn-md btn-primary btn-icon" title="Edit">
                      ...
                      <div class="modal-dialog">
                        <div class="modal-content">
                          ...
                          <div class="modal-footer">
                            <form id="hapus-data" action="{{ route('admin.staff.delete') }}" method="POST" class="d-none">
                              @csrf
